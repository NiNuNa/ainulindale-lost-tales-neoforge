...
import ninuna.losttales.client.gui.mapiconprovider.LostTalesDirectionMapIconProvider;
import ninuna.losttales.client.gui.mapiconprovider.LostTalesHostileMapIconProvider;
import ninuna.losttales.client.gui.mapiconprovider.LostTalesVillageMapIconProvider;
...
public class LostTalesCompassHud {
    ...
    public static final int MAP_MARKER_WIDTH = 11;
    public static final int MAP_MARKER_HEIGHT = 11;
    public static final int MAP_MARKER_OFFSET_Y = 5;
    ...
    private static final List<IconProvider> PROVIDERS = List.of(
    ...
    private static void drawMapIcons(Minecraft minecraft, GuiGraphics guiGraphics, int compassCenterX, int compassY, float playerYaw, float pxPerDeg, int visibleDegRange) {
        ...
        for (IconProvider provider : PROVIDERS) {
            try {
                List<MapIcon> batch = provider.collect(minecraft);
        ...
        record RenderItem(MapIcon icon, int px, float edgeT, float centerEmphasis, float alphaT, int color, float baseScale) {}
        List<RenderItem> renderItems = new ArrayList<>(icons.size());

        // Focus candidate tracking (single winner)
        MapIcon bestIcon = null;
        int bestPx = 0;
        ...
        for (MapIcon icon : icons) {
            ...
            if (better) {
                ...
                if (icon.hasRelative()) {
                    bestDx = icon.dx();
                    bestDy = icon.dy() != null ? icon.dy() : 0.0;
                    bestDz = icon.dz();
                } else {
        ...
        for (RenderItem ri : renderItems) {
            ...
            guiGraphics.blit(RenderType::guiTextured, icon.customOrDefaultTexture(),
        ...
        if (bestIcon != null && bestEmphasis > 0f) {
            ...
            int labelColor = alphaColor(LostTalesGuiUtil.COLOR_WHITE_FADE, bestEmphasis);
    ...
    public interface IconProvider {
        List<MapIcon> collect(Minecraft mc);
    }
    ...
    public record MapIcon(
        ...
        public static MapIcon mapIconAtAngle(String fullName, int argb, int texU, int texV, float angleDeg, ResourceLocation texture) {
            double rad = Math.toRadians(angleDeg);
            double dx = -Math.sin(rad);
            double dz =  Math.cos(rad);
            return new MapIcon(fullName, argb, false, false, 0f, null, null, null, dx, dz, null, texU, texV, texture);
        }

        public ResourceLocation customOrDefaultTexture() {
            return texture != null ? texture : MAP_MARKERS_TEXTURE;
        }

        public boolean hasRelative() {
            return dx != null && dz != null;
        }

        public boolean hasVertical() {
            return dy != null || worldY != null;
        }
